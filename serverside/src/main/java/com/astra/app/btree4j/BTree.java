...
import com.astra.app.btree4j.indexer.BasicIndexQuery;
import com.astra.app.btree4j.indexer.IndexQuery;
import com.astra.app.btree4j.utils.codec.VariableByteCodec;
import com.astra.app.btree4j.utils.collections.longs.LongHash.BucketEntry;
import com.astra.app.btree4j.utils.collections.longs.LongHash.Cleaner;
import com.astra.app.btree4j.utils.collections.longs.PurgeOptObservableLongLRUMap;
import com.astra.app.btree4j.utils.io.FastMultiByteArrayOutputStream;
import com.astra.app.btree4j.utils.lang.ArrayUtils;
import com.astra.app.btree4j.utils.lang.Primitives;
...
public class BTree extends Paged {
    ...
    static {
        DEFAULT_IN_MEMORY_NODES =
                Primitives.parseInt(Settings.get("com.astra.app.btree4j.btree.nodecache_size"), 4096); // 16m
        BTREE_NODECACHE_PURGE_UNIT =
                Primitives.parseInt(Settings.get("com.astra.app.btree4j.bfile.nodecache_purgeunit"), 8); // 32k
    }
    ...
    private final void scanRange(@Nonnull BTreeNode left, @Nonnull BTreeNode right,
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("scan range end. total scaned pages: " + scaned);
        }
    }
    ...
    private final BTreeNode getBTreeNode(BTreeRootInfo root, long page, @Nullable BTreeNode parent)
        ...
        synchronized (_cache) {
            ...
            if (node == null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("read node page#" + page + ", keys: " + node.keys.length);
                }
    ...
    private final class BTreeNode implements Comparable<BTreeNode> {
        ...
        private void write() throws IOException, BTreeException {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace((ph.getStatus() == LEAF ? "Leaf " : "Branch ") + "Node#"
